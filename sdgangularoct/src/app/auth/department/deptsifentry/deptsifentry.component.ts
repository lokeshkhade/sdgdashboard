...
@Component({
  selector: 'app-deptsifentry',
  templateUrl: './deptsifentry.component.html',
  styleUrls: ['./deptsifentry.component.scss']
})
export class DeptsifentryComponent implements OnInit {


  public sifentry: FormGroup; //add  FormGroup 
  ...
  constructor(private fb: FormBuilder, private ds: DataService, private authService: AuthService, private dp: DatePipe) {
    this.sifentry = this.fb.group({ //definition to cons
      Year: [],
      sifentry: new FormArray([])
    });
  }
  ...
  get f() { return this.sifentry.controls; }
  get t() { return this.f['sifentry'] as FormArray; }
  ...
  {
    return this.sifentry.controls[fieldName].invalid &&
      (this.sifentry.controls[fieldName].dirty || this.sifentry.controls[fieldName].touched);
  }
  ...
  showdatapoint() 
  {
    if (this.selectedyear == "") 
    {
      ...
      this.ds.param2Function('common/getsifvaluesbyyear', this.deptid, this.selectedyear).subscribe((res: any) => {
        this.indicators = res;
        if (this.indicators == 'Data Not Found') //Check 
        {
          ...
          this.t.clear();
          this.ds.paramFunction('common/getsifdeptindicator', this.deptid).subscribe((res: any) => {
            this.indicators = res;
            for (let i = 0; i < this.indicators.length; i++) {
              this.t.push(this.fb.group({
                indicator_id: this.indicators[i].indicator_id,
                IndicatorValue: +this.indicators[i].IndicatorValue
              }));
            }
          });
        }
  ...
  onSubmit() 
  {
    this.sifentry.patchValue
      ({
        Year: this.selectedyear
      });
      this.ds.postData('crud/sifinsert', this.sifentry.value).subscribe(res => 
      {
        this.data = res;
        if (this.data) 
        {
  ...
  onUpdate() 
  {
    ...
    console.log("loku;", this.sifentry.value.sifentry[0].IndicatorValue);
    this.ds.updateData('crud/sifinsert', this.sifentry.value).subscribe(res => 
    {
      this.data = res;
      if (this.data) 
      {
